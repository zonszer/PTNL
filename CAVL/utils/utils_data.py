...
def prepare_cv_datasets(dataname, batch_size):
    ...
    if dataname == 'mnist':
        ordinary_train_dataset = dsets.MNIST(root='CAVL/'+datapath, train=True, transform=transforms.ToTensor(), download=True)
        test_dataset = dsets.MNIST(root='CAVL/'+datapath, train=False, transform=transforms.ToTensor())
    elif dataname == 'kmnist':
        ordinary_train_dataset = dsets.KMNIST(root='CAVL/'+datapath, train=True, transform=transforms.ToTensor(), download=True)
        test_dataset = dsets.KMNIST(root='CAVL/'+datapath, train=False, transform=transforms.ToTensor())
    elif dataname == 'fashion':
        ordinary_train_dataset = dsets.FashionMNIST(root='CAVL/'+datapath, train=True, transform=transforms.ToTensor(), download=True)
        test_dataset = dsets.FashionMNIST(root='CAVL/'+datapath, train=False, transform=transforms.ToTensor())
        ...
        test_transform = transforms.Compose(
            [transforms.ToTensor(),
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.247, 0.243, 0.261))])
        ordinary_train_dataset = dsets.CIFAR10(root='CAVL/'+datapath, train=True, transform=train_transform, download=True)
        test_dataset = dsets.CIFAR10(root='CAVL/'+datapath, train=False, transform=test_transform)
